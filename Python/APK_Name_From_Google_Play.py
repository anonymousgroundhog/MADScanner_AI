...
def get_apk_info(directory_path, output_filepath)
    ...
    extracts their  name using 'aapt dump badging', fetches the
            ...
            for file in files
                if file.endswith(".apk")
                    ...
                    try
                        ...
                        # --- Step 2 Get  name using aapt dump badging ---
                        command = ["aapt", "dump", "badging", apk_filepath]
                        result = subprocess.run(command, capture_output=True, text=True, check=True, encoding='utf-8')
                        output = result.stdout

                        # Extract  name using regex
                        _match = re.search(r" name='([^']+)'", output)
                        if _match
                            _name = _match.group(1)

                        # --- Step 3 Fetch app name from Google Play Store using  name ---
                        if _name != 'N/A'
                            try
                                # Fetch app details from Google Play Store
                                play_store_info = app(_name, lang='en', country='us')
                                app_name = play_store_info.get('title', 'Not found on Play Store')
                            except exceptions.NotFoundError
                                app_name = "Not found on Play Store ( not found)"
